...
class WP_Quiz_Page_Import_Export {
	...
	public static function admin_print_styles() {
		wp_enqueue_style( 'pro-popup-css', WP_QUIZ_ASSETS_URL . 'css/pro-popup.css', array(), WP_QUIZ_VERSION );
		?>
			<style>
	...
	public static function display_messages(){
		...
		if ( isset( $_REQUEST[ 'message' ] ) && ( $msg = (int) $_REQUEST[ 'message' ] ) ) {
			if (  $msg == 3 )
				$message = __( 'Quiz imported successfully', 'wp-quiz' );
			else if ( $msg == 4 )
				$message = __( 'Failed to import Quiz', 'wp-quiz' );
		}
	...
	public static function load(){
		...
		//bootstrap js
		wp_enqueue_script( 'wp_quiz-bootstrap', WP_QUIZ_ASSETS_URL . 'js/bootstrap.min.js', array( 'jquery' ), WP_QUIZ_VERSION, true );
	...
	public static function page(){
		...
		if ( $columns ) {
			...
			<div class="wrap" id="config-page">
				<h2><?php _e( 'Import/Export', 'wp-quiz' ); ?></h2>
				...
				<?php include_once( WP_QUIZ_PATH . '/inc/pro-popup-template.php' ); ?>
			</div>
		...
		?>
			<div>
				<ul id="tabs" class="tabs" data-tabs="tabs">
					<li class="active"><a href="#import" data-toggle="tab"><?php _e( 'Import', 'wp-quiz' ); ?></a></li>
					<li><a id="export_tab" href="#export" data-toggle="tab"><?php _e( 'Export', 'wp-quiz' ); ?></a></li>
				</ul>
				<div class='tab-content'>
					<div class="tab-pane active" id="import">
							...
							<div>
								<div class="demo-container">
									<div class="demo-thumb">
										<a href="http://demo.mythemeshop.com/wp-quiz/" target="_blank"><img src="<?php echo WP_QUIZ_BASE_URL .'/demo/import.jpg'; ?>" /></a>
									</div>
								</div>
							</div>
							<p>
								...
								<input type="submit" name="submit" id="submit" class="button-primary" value="<?php _e('Import Demo','wp-quiz'); ?>">&nbsp;
							</p>
						</form>
						<hr/>
						<p><?php _e( 'Import Quiz File', 'wp-quiz' ); ?></p>
						<form action="<?php echo admin_url( 'edit.php?post_type=wp_quiz&page=wp_quiz_ie' ); ?>" method="post" enctype="multipart/form-data">	
							...
							<p>
								<input type="hidden" name="action" value="import" />
								<input type="submit" name="submit" id="submit" class="button-primary" value="<?php _e('Import File','wp-quiz'); ?>">&nbsp;
							</p>
						</form>
					</div>
					<div class="tab-pane" id="export">
						<p><?php _e('To use this feature please upgrade to pro version.','wp-quiz'); ?></p>
					</div>
				</div>
			</div>
		<?php
	...
	public static function import_wp_quiz(){
		...
		$quizObj	= file_get_contents( $wq_upload_dir[ 'baseurl' ] . '/'. $imported_file[ 'name' ] );

		$quizObj	= json_decode( $quizObj, true );

		foreach( $quizObj as $qok => $qov ){
			// Questions
	...
	public static function import_wp_quiz_demo(){
		...
		// Get JSON File and It's contents
		$quizObj	= file_get_contents( WP_QUIZ_BASE_URL . '/demo/wp_quiz_demo.json' );
