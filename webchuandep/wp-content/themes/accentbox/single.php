				...
				<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
					...
					<?php } ?>
						...
						<div class="social">
							...
							<div id="fb-root"></div>
							<script src="http://connect.facebook.net/en_US/all.js#appId=193109300735690&amp;xfbml=1"></script><fb:like href="<?php the_permalink(); ?>" send="false" show_faces="false" width="320" style="margin: 1.3em 0 0 1.2em;"></fb:like>
						</div>
						<?php if($options['mts_author_box'] == '1') { ?>
							<div class="postauthor">
								<?php if(function_exists('get_avatar')) { echo get_avatar( get_the_author_meta('email'), '90' );  } ?>
								<h4>Article by <a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>"><?php the_author_meta( 'nickname' ); ?></a></h4>
						...
						<?php if($options['mts_related_posts'] == '1') { ?>
								...
								<?php
								} echo '</ul></div>'; } } wp_reset_query(); ?> <!-- .related-posts -->
							<?php }?>
					</div><!--.g post-->
