					...
					<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
						<div class="post excerpt <?php echo (++$j % 2 == 0) ? 'last' : ''; ?>">
							<header>
								...
								<div class="post-info">
								Posted in <?php $category = get_the_category(); echo '<a href="'.get_category_link($category[0]->cat_ID).'">'.$category[0]->cat_name.'</a>';?> by <?php the_author_posts_link(); ?> on <?php the_time('F j, Y'); ?>
								</div>
							</header><!--.header-->
