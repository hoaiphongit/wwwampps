    ...
    $("#setting-error-tgmpa").on("click", ".notice-dismiss", function(e) {
        ...
        public function register( $plugin ) {
            ...
            if ( !empty ( $plugin['available_separately'] ) ) {
                $item['source'] = __( 'Available Separately', 'tgmpa' );
            }
    ...
    class TGMPA_List_Table extends WP_List_Table {
        ...
        public function get_views() {
            ...
            foreach ( $this->view_totals as $type => $count ) {
                ...
                switch ( $type ) {
                    case 'all':
                        $text = wp_kses( _nx( 'All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>', $count, 'plugins', 'tgmpa' ), array( 'span' ) );
                        break;
                    case 'install':
                        $text = wp_kses( _n( 'To Install <span class="count">(%s)</span>', 'To Install <span class="count">(%s)</span>', $count, 'tgmpa' ), array( 'span' ) );
                        break;
                    case 'update':
                        $text = wp_kses( _n( 'Update Available <span class="count">(%s)</span>', 'Update Available <span class="count">(%s)</span>', $count, 'tgmpa' ), array( 'span' ) );
                        break;
                    case 'activate':
                        $text = wp_kses( _n( 'To Activate <span class="count">(%s)</span>', 'To Activate <span class="count">(%s)</span>', $count, 'tgmpa' ), array( 'span' ) );
                        break;
                    default:
                        $text = '';
                        break;
        ...
        public function column_description( $item )
        {
            ...
            $installed = ! empty( $item['installed_version'] ) ? $item['installed_version'] : __( 'Not Installed', 'tgmpa' );
            $description .= sprintf( __( 'Version: %s', 'tgmpa' ), $installed );
            $description .= ' | ' . sprintf( __( 'Source: %s', 'tgmpa' ), $item['source'] );

            if ( ! empty( $item['info_link'] ) ) {
                $description .= ' | ' . sprintf( '<a target="_blank" href="%s">' . __( 'Visit plugin site', 'tgmpa' ) . '</a>', $item['info_link'] );
            }
        ...
        public function no_items() {
            printf( wp_kses( __( 'No plugins to install, update or activate. <a href="%1$s">Return to the Dashboard</a>', 'tgmpa' ), array( 'a' => array( 'href' => array() ) ) ), esc_url( self_admin_url() ) );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        }
        ...
        public function get_columns() {
            $columns = array(
                ...
                'description' => __( 'Description', 'tgmpa' ),
            );
                ...
                class TGMPA_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                    ...
                    public function add_strings() {
                        if ( 'update' === $this->options['install_type'] ) {
                            ...
                            $this->upgrader->strings['skin_update_failed_error'] = wp_kses( __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'tgmpa' ), array( 'strong' ) );
...
function mts_register_required_plugins() {
    ...
    $plugins = array(
        ...
        array(
            ...
            'description' => __( 'A simple and easy way to update your MyThemeShop themes and plugins to the the latest versions using good old one click method!', 'tgmpa' ),
            'required'  => false,
        ...
        array(
            ...
            'alt_description' => __( 'Create reviews! Choose from Stars, Percentages, Circles or Points for review scores. Supports Retina Display, WPMU & Unlimited Color Schemes.', 'tgmpa' ),
        ...
        array(
            ...
            'alt_description' => __( 'WP Notification Bar is a custom notification and alert bar plugin for WordPress which is perfect for marketing promotions, alerts, increasing click throughs to other pages and so much more.', 'tgmpa' ),
        ...
        array(
            'name'      => 'WP Mega Menu',
            'slug'      => 'wp-mega-menu',
            'description' => __( 'WP Mega Menu is an easy to use plugin for creating beautiful, customized menus for your site. With no setup required, tons of options to choose from, and the ability to show categories, subcategories and posts, WP MegaMenu is a must have plugin that also boosts SEO and user engagement.', 'tgmpa' ),
...
function mts_tgmpba_table_data_item( $item, $plugin ) {
    if ( ! empty ( $plugin['available_separately'] ) ) {
        $item['source'] = __( 'MyThemeShop', 'tgmpa' );
        ...
        <form id="tgmpa-plugins" action="" method="post">
            ...
            <div class="wrap mts-addons">
                ...
                <header class="mts-addons-header">
                    <h2><?php _e( 'Recommended Plugins', 'tgmpa' ); ?></h2>
                    <p><?php printf( __( 'MyThemeShop and third party plugins recommended for %s theme!', 'tgmpa' ), '<a href="'.esc_url( $theme_uri ).'"><strong>'.$theme_name.'</strong></a>' ); ?></p>
                </header>
                ...
                if ( isset( $_GET['mts_plugins_list'] ) ) {
                    ...
                    foreach ( $plugins as $plugin ) :
                        ...
                        if ( isset( $plugin['alt'] ) ) {
                            ...
                            // Free
                                ...
                                if ( $tgma_instance->is_plugin_installed( $plugin['alt'] ) ) {
                                    ...
                                    $go_pro_button = '<a class="mts-addon-gopro-button button button-primary" href="' . esc_url( $plugin['info_link'] ) . '" target="_blank">' . __( 'Get Pro Version!', 'tgmpa' ) . '</a>';
                                }
                            }
                        }
                        ...
                        if ( $tgma_instance->is_plugin_installed( $plugin['slug'] ) ) {
                            ...
                            if ( is_plugin_active( $plugin['file_path'] ) ) {
                                ...
                                $status         = 'active';
                                $status_message = __( 'Active', 'tgmpa' );
                                $url            = admin_url( 'plugins.php' );
                                $button = '<a class="mts-addon-button button" href="' . esc_url( $url ) . '">'.__('Manage Plugin', 'tgmpa' ).'</a>';

                            } else {

                                $status         = 'inactive';
                                $status_message = __( 'Inactive', 'tgmpa' );
                                $url = wp_nonce_url(
                                    add_query_arg(
                                        array(
                            ...
                            if ( !empty( $plugin['available_separately'] ) || isset( $plugin['pro'] ) ) {
                                ...
                                $text   = __( 'Install Plugin', 'tgmpa' );
                                $target = '';
                            }
                           
                            $class = 'mts-addon-button button button-primary';
                            $status         = 'not-installed';
                            $status_message = __( 'Not Installed', 'tgmpa' );
                    ...
                    <ul class="mts-addons-list loading" id="mts-addons-list">
                        <li class="loader"><?php _e( 'Loading...', 'tgmpa' ); ?></li>
                    </ul>
                    <?php
                } 
                ?>
